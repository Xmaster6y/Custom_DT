...
def _step(tensordict):
    ...
    board = chess.Board(fen=translate.complete_tensor_to_fen(board_tensor.int()))  # needs dtype = int64 & shape = (72,)
...
def stockfish_move(board_tensor):
    board = chess.Board(fen=translate.complete_tensor_to_fen(board_tensor.int()))
...
def simple_rollout(steps=10):
    data = TensorDict({}, [steps])
    _data = env.reset()
    for i in range(steps):
        print(i)
        _data["action"] = stockfish_move(_data["board"])  # will replace with DT next time I work.
...
class ChessEnv(EnvBase):
    ...
    def __init__(self, td_params=None, seed=None, device="cpu", batch_size=None):
        if td_params is None:
            td_params = gen_params(batch_size=batch_size)
