...
def validate_params(parameter_name, input, expected_type, permitted_values: list):
    """
    ...
    if permitted_values:
        if input not in permitted_values:
            raise ValueError(f"Invalid value for parameter '{parameter_name}'. Permitted values: " + str(permitted_values))


def stockfish_eval(boards: list, engine: chess.engine.SimpleEngine, player: str = 'white', evaluation_depth: int = 8) -> list:
    """
