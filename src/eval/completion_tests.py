    ...
    def two_player_prep(self, seq):
        ...
        if self.shaping:
            ...
            for (b_idx, b) in enumerate(boards):
                info = self.engine.analyse(b, chess.engine.Limit(depth=self.shaping_eval_depth), info=chess.engine.Info.SCORE)
                if b_idx % 2 == 0:
...
if __name__ == "__main__":
    ...
    CT = CompletionTest(file_name=FILE_NAME, n_test_games=N, state_dim=STATE_DIM, action_dim=ACT_DIM, discount=DISCOUNT, shaping=SHAPING, single_player=SINGLE_PLAYER, device=DEVICE)
    CT.prepare_data()
    # CT.run_test()
